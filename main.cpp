...
int main() {
  ...
  std::unique_ptr<InputMode> currentMode = std::make_unique<SelectionMode>(Context{chunkStorage, entities, window, gameview});
...
// ------------------------------- MAIN LOOP -----------------------------------
  ...
  while (window.isOpen())
  {
    ...
    // chunkStorage.map[mpos].c[hex_in_chunk_pos.x][hex_in_chunk_pos.y].s.setFillColor(sf::Color(32, 31, 35));
