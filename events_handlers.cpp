...
std::unique_ptr<InputMode> SelectionMode::handleEvent(Context &context, sf::Event event) {
  switch (event.type) {
...
std::unique_ptr<InputMode> SelectionMode::handleMouseEvent(Context &context, sf::Event event) {
...
std::unique_ptr<InputMode> SelectionMode::handleKeyPress(Context &context, sf::Event event) {
...
std::unique_ptr<InputMode> MovementMode::handleEvent(Context &context, sf::Event event) {
  switch (event.type) {
...
std::unique_ptr<InputMode> MovementMode::handleMouseEvent(Context &context, sf::Event event) {
  switch (event.mouseButton.button) {
    case sf::Mouse::Left:
      return handleLeftClick(context);
    case sf::Mouse::Right:
...
std::unique_ptr<InputMode> MovementMode::handleKeyPress(Context &context, sf::Event event) {
  switch (event.key.code) {
    case sf::Keyboard::Escape:
      return std::make_unique<SelectionMode>(context);
    default:
...
CreationMode::CreationMode(Context context) {
  sf::Vector2i anchor = hex_from_pix(context.window.mapPixelToCoords(sf::Mouse::getPosition(context.window)));
  preview = Entity(anchor.x, anchor.y, "previewplayer", 0, 0);
}

void CreationMode::updateContext(Context context) {
  sf::Vector2i sizeOrigin = hex_from_pix(context.window.mapPixelToCoords(sf::Mouse::getPosition(context.window)));
  sf::Vector2i ppos = preview.getPos();
  sf::Vector2i baseDelta;
  baseDelta.x = (sizeOrigin.x - ppos.x >= 0);
  baseDelta.y = (sizeOrigin.y - ppos.y >= 0);

  if ((baseDelta.x + sizeOrigin.x - ppos.x != preview.getWidth()) || (baseDelta.y + sizeOrigin.y - ppos.y != preview.getHeight())) {
    preview = Entity(preview.getName(), ppos.x, ppos.y, preview.getType(), baseDelta.x + sizeOrigin.x - ppos.x, baseDelta.y + sizeOrigin.y - ppos.y); // shit idk where that 1 is coming from
                                                                // WAIT ik : when theyre the same : it should be (1,1) so : (1,1)+(whatever)
  }
  preview.setView(context.view);
}

std::unique_ptr<InputMode> CreationMode::handleEvent(Context &context, sf::Event event) {
  switch (event.type) {
...
std::unique_ptr<InputMode> CreationMode::handleMouseEvent(Context &context, sf::Event event) {
  switch (event.mouseButton.button) {
    case sf::Mouse::Left:
...
std::unique_ptr<InputMode> CreationMode::handleKeyPress(Context &context, sf::Event event) {
  switch (event.key.code) {
    case sf::Keyboard::Escape:
      return std::make_unique<SelectionMode>(context);
    default:
...
void CreationMode::draw(sf::RenderTarget &target, sf::RenderStates states) const {
  target.draw(preview, states);
}
