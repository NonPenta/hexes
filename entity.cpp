...
Entity::Entity(std::string name, int x, int y, std::string type_, int width_, int height_) : name{name}, pos{x, y}, type{type_}, width{width_}, height{height_}, selected{0}, members{}, viewPos{}, viewSizeDelta{} {
  for (int xi = x; xi < x+width_; xi++) {
    for (int yi = y; yi < y+height_; yi++) {
      members[std::pair<int,int>{xi, yi}] = Hex(xi, yi, type_);
    }
